...
- [x] PMI — ECOS: 제조업 PMI 전용 표 미제공 구간 대응 → 전산업 업황실적 BSI(`512Y007`, 항목 AA·99988)로 대리, 기준금리는 `722Y001` 일간 검증
- [x] 환율 (원/달러, ECOS)
...
- [x] 어닝 리비전 방향성 — 섹터 선행 신호(가격·ETF 자금·동종PER 프록시, 컨센 미연동 시 안내)
...
- [x] 멀티팩터 스코어 (가치+모멘텀+퀄리티+저변동성) — 단일 종목 0~100 복합 및 세부 (`quant_agent`); 전종목 순위는 유니버스 배치 시
- [x] 컨센서스 괴리율 — 동종 PER/PBR 중앙값 내재가 vs 현재가 괴리 (`quant_agent` `consensus_gap_proxy`, 애널 목표주가 API 無 시 대용)
- [x] 어닝 서프라이즈 이력 — 연간 당기순이익 YoY·가속도·플러스 연속년 (`earnings_surprise_proxy`, 컨센 대비 실제 서프라이즈는 별도 데이터 필요 안내)
- [x] 내부자 거래 신호 — DART 공시 제목 휴리스틱(자사주 취득·처분·주요주주 관련) (`insider_disclosure_hints`)
...
- [x] 매수/매도/홀딩 우선순위 조언 (`action_priorities`)
- [x] 시장 국면별 방어 전략 제시 (`market_regime_hint`, 코스피 60일 수익률 휴리스틱)
