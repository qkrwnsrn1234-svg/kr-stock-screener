...
**현재 브랜치**: main
**다음 할 일**: 퀀트 멀티팩터·섹터 어닝 리비전·어드바이저 우선순위 등 미체크 항목 — ECOS는 기준금리 `722Y001`·PMI→전산업 BSI(`512Y007`)로 실데이터 검증 반영됨
**최종 배포 목표**: Phase 5 — `make build-desktop && make install` 로 `/Applications` 설치·첫 실행 보안 안내까지 반영
**마지막 커밋**: ECOS 통계표 실데이터 교정 — `722Y001` 기준금리, `512Y007` 전산업 BSI(PMI 대리), `macro_agent` BSI 임계값 100
